...
JSON-RPC 约定：
- 请求：`{"jsonrpc":"2.0","id":1,"method":"xxx","params":{...},"unixtime_ms":1710000000000}`
- 成功响应：`{"jsonrpc":"2.0","id":1,"result":{...}}`
- 错误响应：`{"jsonrpc":"2.0","id":1,"error":{"code":-32000,"message":"..."}}`
- 通知（无 id）：`{"jsonrpc":"2.0","method":"xxx","params":{...}}`
- 服务端响应统一附带：`request_unixtime_ms`、`response_unixtime_ms`

## 运行流程（当前实现）
1. 服务端启动，监听 `/ws/mail`，并启动空闲检测线程（每 30 秒检查连接数）
2. 客户端连接服务端
...
- `--client-account`：内部客户端登录账号（默认取邮箱 `@` 前缀；若设置 `WS_CLIENT_ACCOUNT` 则优先使用）
